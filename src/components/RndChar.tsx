...
function RndChar() {
  ...
  const [chars, setChars] = useState<ALCharType[]>([]);

  // Contendrá el objeto con los datos del personaje preguntado.
  const [currChar, setCurrChar] = useState<ALCharType | null>(null);
  ...
  const [score, setScore] = useState<AppScoreType>({
    total: 0,
    correct: 0,
  });
  ...
  const [transl, setTransl] = useState<TranslType | null>(null);
  ...
  const getData = (query: string, variables = {}): Promise<ApiResponseType> => {
  ...
  const getRandomChars = (): void => {
    ...
    getData(query, variables)
      .then((response) => {
        const data: ALCharType[] = response.data.Page.characters;
        const charsData: ALCharType[] = [];
  ...
  const checkAnswer = (char: ALCharType): void => {
